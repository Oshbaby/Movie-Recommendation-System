...
def fetch_poster(movie_id):
    url = f"https://api.themoviedb.org/3/movie/{movie_id}?api_key=d5a816607e20854724cc4415f7ce38a1" #the key is not important to me so enjoy!
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
        poster_path = data.get('poster_path', '')  # Use .get() to prevent KeyError
        if poster_path:
            return f"http://image.tmdb.org/t/p/w500{poster_path}"  # Corrected URL format
    return None    
